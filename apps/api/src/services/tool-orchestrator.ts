...
import { getAveMarketData, searchAveTokens } from './tools/market-data.js';
import { analyzeGrokSentiment } from './tools/grok.js';
import { scrapeGovernance } from './tools/governance-firecrawl.js';
...
export const conversationTools = {
  searchNews: tool({
    ...
    inputSchema: z.object({
      query: z.string().describe('Search query (e.g. "EUR/USD forecast", "crypto stablecoin news")'),
  ...
  getCryptoPrices: tool({
    description:
      'Get current cryptocurrency prices via AVE. Use "address-chain" format (e.g. "0xabc...-bsc").',
    inputSchema: z.object({
      ids: z.array(z.string()).describe('Token IDs in "address-chain" format'),
    }),
    execute: async ({ ids }) => {
      try {
        const prices = await withTimeout(getAveMarketData(ids.slice(0, 10)), TOOL_TIMEOUT_MS);
        return { prices };
      } catch (err) {
  ...
  searchCoins: tool({
    description: 'Search for cryptocurrency by name, symbol, or address via AVE.',
    inputSchema: z.object({
      query: z.string().describe('Search query (e.g. "PEPE", "0xabc...")'),
    }),
    execute: async ({ query }) => {
      try {
        const coins = await withTimeout(searchAveTokens(query), TOOL_TIMEOUT_MS);
        return { coins };
      } catch (err) {
  ...
  analyzeSocialSentiment: tool({
    description:
      'Analyze current X (Twitter) and social media sentiment about a topic. Use for crypto or market sentiment.',
    inputSchema: z.object({
      topic: z.string().describe('Topic to analyze (e.g. "DeFi trends", "stablecoin markets")'),
    }),
    execute: async ({ topic }) => {
      try {
        const xDomains = ['x.com', 'twitter.com'];
  ...
  getGovernance: tool({
    description:
      'Fetch current on-chain governance information. Use for proposals, voting, staking, or governance questions.',
    inputSchema: z.object({}),
    execute: async () => {
      try {
        const data = await withTimeout(scrapeGovernance(), TOOL_TIMEOUT_MS);
        return data;
      } catch (err) {
        return {
          markdown: '',
